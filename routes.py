    ...
    if "user" in session:
        ...
        if post_form.validate_on_submit() and request.form.get('select_vehicle') != 'not selected':
            ...
            create_post_db(session["user"], post_form.car_brand.data, post_form.car_model.data,
                           ...
                           post_form.place_of_departure.data, post_form.destination.data, post_form.price.data, post_form.note.data,
                           post_form.house_to_house.data, post_form.package_delivery.data, request.form.get('select_vehicle'),
            ...
            flash(f'Válassz a listából !', 'danger')
        return render_template('create_post.html', post_form=post_form, today_date=today_date, modal_attr=modal_attr, user=user)
    return redirect(url_for('index'))
...
def reserve():
    if request.method == 'POST':
        ...
        insert_reserved_posts_for_passenger(request.form.get('post_azon'), session['user'], int(request.form.get('free_seats')))
...
def passenger():
    if "user" in session:
        ...
        if roads:
            ...
            return render_template('passenger.html', drivers=posts, user=user, current_reserved_post=current_reserved_post)
        else:
            return render_template('passenger.html', drivers=posts, user=user, current_reserved_post=current_reserved_post)
